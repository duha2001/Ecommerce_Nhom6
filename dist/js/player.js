(function($) {
    ...
    if (isShuffle && shuffle_array != undefined && playlist.length === (shuffleArray = JSON.parse(shuffle_array)).length) {
        ...
        isShuffle = false;
        $('#QPlayer .cover').attr('title', 'Nhấp để bắt đầu trộn');
    }
    ...
    $("#QPlayer .ssBtn").on('click', function() {
        ...
        if ($('.ssBtn .adf').hasClass('on') === false) {
            if (isFirstPlay) {
                setTimeout("showTips('#player .cover','Nhấp vào bìa để bật (tắt) trộn bài', " + function() { setTimeout("showTips('#player .ctrl .musicTag','Nhấp và kéo thanh tiêu đề để tua đi (tua lại)')", 1000) } + ");", 500);
    ...
    $("#player .cover").on('click', function() {
        ...
        if (isShuffle) {
            ...
            $("#player .cover").attr("title", "Nhấp để bắt đầu trộn");
            showNotification('Phát ngẫu nhiên đã tắt');
...
function showTips(div, info, func) {
    var box_height = 100;
    $('body').append('<div class="qplayer_tips"><span class="tips_arrow"></span><span class="info" style="display:none">' + info + '</span><button class="tips_button" onclick="removeTips()">Đóng</button></div>');
